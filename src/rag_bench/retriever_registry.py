...
_RETRIEVER_SPECS: tuple[RetrieverSpec, ...] = (
    ...
    RetrieverSpec(
        id="agent",
        label="Agent",
        description="Deterministic agent-style text retrieval wrapper using BM25 query variants, RRF, and downstream dictionary tool fallback.",
        requires_extra=None,
        factory=lambda _context: MultiQueryRetriever(name="agent"),
        category="text",
        uses_llm=True,
    ),
    RetrieverSpec(
