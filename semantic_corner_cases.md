...
| Viết tắt trong câu hỏi ngắn | `PB`, `P B`, `pblagi`, `giaithichpb`, `PB viết tắt cho gì?`, `pbviettatcuagi`, `CVHL nghĩa là gì?`, `cvhlnghialagi`, `KHCN xuất hiện ở đâu?`, `khcnxuathienodau`, `CTCC xuất hiện ở đâu?`, `ctccxuathienodau`, `Q S P B` | Planner phải normalize các biến thể lookup ngắn về cùng target acronym/term gốc, rồi dictionary retrieval dùng target đó để giữ kết quả nhất quán trong cả Dictionary mode, `/v1/dictionary/lookup`, và Text-only dictionary fallback; không để câu có hậu tố `là gì`/`nghĩa là gì`/`mean`/`viết tắt cho gì`/`xuất hiện ở đâu`, tiền tố `giải thích`, viết dính, hoặc thừa khoảng trắng rơi sang lexical weak match hoặc benchmark fallback. Với chuỗi compact như `ctccxuathienodau`, headword rất ngắn như `NỔ` không được match substring và vượt entry thật có nhắc `CTCC`. | `tests/test_dictionary_query_planner.py::test_definition_plan_strips_question_noise_for_short_acronyms`, `tests/test_retrievers.py::test_dictionary_graph_retriever_does_not_promote_short_headword_inside_compact_lookup`, `tests/test_chat_service.py::test_dictionary_lookup_uses_normalized_lookup_target_for_abbreviation_questions`, `tests/test_chat_service.py::test_dictionary_mode_normalizes_short_acronym_definition_queries`, `tests/test_chat_service.py::test_text_mode_dictionary_fallback_uses_normalized_lookup_target_for_mentions` |
| Tracking lớp normalization | Synthetic/redacted `AB`, `XYZ`, `T7K`, câu negative có `XYZ` nhưng không phải lookup | Không thêm từng cụm PB/CVHL/KHCN/CTCC vào code như rule riêng lẻ; mỗi lớp normalizer phải có metadata layer/adapter và có thể so sánh bằng fixture an toàn. | `tests/test_dictionary_query_planner.py::test_short_lookup_normalization_generalizes_across_unseen_targets`, `tests/test_dictionary_query_planner.py::test_short_lookup_normalization_accepts_pluggable_adapter_memory`, `scripts/evaluate_dictionary_normalization_layers.py` |
...
| Viết tắt kèm phrasing lookup | `PB`, `PB là gì?`, `PB viết tắt cho gì?`, `PB là viết tắt của gì?`, `giải thích PB`, `CVHL nghĩa là gì?`, `cho tôi biết CVHL là gì`, `what does SPG9 mean?` | Các câu cho kết quả khác nhau: câu trần có direct alias/headword hit, còn câu có wrapper hỏi đáp bị tách target thành cả cụm câu hỏi nên rơi sang lexical weak match hoặc context benchmark cũ. | Mọi biến thể lookup ngắn phải được coi là cùng một target acronym/term gốc; kết quả phải ưu tiên canonical abbreviation/headword evidence trước alias phụ hoặc lexical mention. Các câu “viết tắt cho/của gì” được xử lý như expansion/definition lookup có bằng chứng, không phải suy luận alias lỏng. | Eval nên kiểm cùng một thuật ngữ dưới nhiều phrasing ngắn, không chỉ test bare acronym; answer không được đổi nghĩa chỉ vì người dùng thêm “là gì”, “nghĩa là gì”, “viết tắt cho gì”, “giải thích”, hoặc “mean”. |
...
| Viết tắt mention bị nhiễu bởi headword ngắn | `CTCC`, `CTCC là gì?`, `CTCC xuất hiện ở đâu?`, `ctccxuathienodau`, `C T C C` | Query viết dính `ctccxuathienodau` có thể bị headword ngắn như `NỔ` match substring trong chuỗi dài và đứng trên nguồn thật. | `CTCC` phải kéo được mục `CÔNG TÁC KHOA HỌC QUÂN SỰ pháo binh`, nơi token này xuất hiện trong nội dung; câu có wrapper hoặc viết dính phải được normalize về target `CTCC`. | Đây là case occurrence lookup giống `KHCN`, nhưng thêm guardrail chống partial substring của headword 1-2 ký tự trong query compact. |
