...
| Viết tắt nhiều nghĩa | `BĐC`, `BĐC tháo súng`, `BĐC 1951` | Bare acronym phải hiện entry kỹ thuật có evidence mạnh trước entry dạng tiểu sử nếu user không nêu ngữ cảnh; autoresearch sinh thêm context probes để kiểm tra disambiguation khi cùng viết tắt có nhiều nghĩa. | `tests/test_retrievers.py::test_dictionary_graph_retriever_does_not_promote_person_initials_over_technical_abbreviation`, `tests/test_dictionary_autoresearch.py::test_red_generator_adds_abbreviation_collision_context_cases` |
| Wrapper tra cứu tự nhiên | `pb là gì`, `tra BĐC`, `BTCN trong từ điển pháo binh` | Các từ bọc truy vấn như `tra`, `là gì`, `trong từ điển pháo binh` không được làm bẩn direct dictionary key. | `tests/test_retrievers.py::test_dictionary_graph_retriever_strips_lookup_wrappers_for_abbreviations` |
| Viết tắt có dấu trong nội dung | `ÂK` | Entry có token exact `ÂK` trong nội dung (`ẨM KẾ`) phải thắng entry chỉ có folded/inferred `AK` (`ÁP KẾ`). | `tests/test_retrievers.py::test_dictionary_graph_retriever_prefers_strict_accented_abbreviation_text_evidence` |
...
| Viết tắt quân sự | `pb`, `pbbc` | `pb` bị match vào chuỗi kỹ thuật như `THƯỚC PB-74`, hoặc `pbbc` bị tách thành token rời và kéo nhầm các mục có chữ `PB`. | `PB` map về `PHÁO BINH`; `PBBC` map về `PHÁO BINH BIÊN CHẾ`, nhờ alias/abbreviation evidence. | Prompt/eval nên yêu cầu giữ nguyên viết tắt, giải thích alias nếu có bằng chứng, và không tự mở rộng viết tắt khi dictionary không hỗ trợ. |
| Viết tắt nhiều nghĩa trong cùng tài liệu | `BĐC` | Chỉ nhìn chữ cái đầu khiến mục nhân vật `BÙI ĐÌNH CƯ` lấn mục kỹ thuật `BÀN ĐẾ CỐI`, hoặc ngược lại, mà không xét ngữ cảnh. | Bare query ưu tiên evidence kỹ thuật đáng tin; query có ngữ cảnh như `BĐC 1951` hoặc `BĐC tháo súng` phải dùng ngữ cảnh để chọn đúng entry. | Eval không nên coi mọi bare acronym collision là một ground truth tuyệt đối; nên sinh thêm context probes và bắt rank #1 ở các context đó. |
...
| Dictionary raw-entry fallback | `/dict AMONIT` với LLM trả rỗng hoặc lỗi provider | Vẫn trả `Mục từ gốc [...]` và nội dung raw dictionary entry; phần `Giải thích` chỉ thêm khi model sinh được nội dung. | `tests/test_chat_service.py::test_dictionary_mode_returns_raw_entry_when_generation_is_empty`, `tests/test_chat_service.py::test_dictionary_mode_falls_back_to_raw_entry_on_generation_error` |
...
- Autoresearch đã có context probes cho dictionary disambiguation khi một viết tắt có nhiều nghĩa, nhưng vẫn cần chạy nhiều vòng trên full artifact để tìm thêm cụm mồi tự nhiên tốt hơn cho từng collision.
