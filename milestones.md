...
- Convert `scripts/run_retriever_diversity_budgetrag_matrix.sh` from a print-only scaffold into an executable runner with retrieval-only default mode and opt-in MiMo generation mode.
- Standardize future MiMo 2.5 jobs on `mimo-v2.5` and disable `mimo_v25_pro` in the default BudgetRAG generation model matrix while keeping historical Pro labels in prior reports.
- Rename the future `mimo_v25` role to `long-context-judge-generator` and require judge/generator model provenance in merged summaries so Pro-vs-standard drift is annotated instead of silent.
- Add `scripts/analyze_context_policy_evidence_quality.py` to summarize context-label evidence quality by retriever, context policy, selected policy, and retrieval-policy combinations before training richer selectors.
- Run the first retriever-diverse retrieval-only SciFact matrix with `bm25`, `graph-bm25`, and `hybrid-rrf`: 45 jobs over 50 queries produced 2250 normalized action rows with full per-query retriever/action coverage, documented in `docs/reports/phase1d_retriever_diversity_retrieval_only_limit50.md`.
- Run the first retriever-diverse MiMo V2.5 generation and answer-label subset: 30 jobs over 10 SciFact queries produced 300 action rows, 300 MiMo answer labels, 186 clean AI-judge reward rows, and 1559 preference pairs; document the result and the low-cap empty-answer issue in `docs/reports/phase1d_retriever_diversity_generation_mimo10.md`.
- Add `scripts/validate_retriever_diversity_generation_subset.py` and `scripts/analyze_retriever_diversity_answer_quality.py`, then publish separate retriever-diversity validation, answer-label, action-coverage, and answer-quality reports for the 300-row MiMo V2.5 subset.
- Complete full 300-row MiMo V2.5 context labeling for the retriever-diverse generation subset, validate 253 clean usable labels with no missing/unknown/duplicate action ids, publish context-label/evidence-quality reports, and run the non-default context reward candidate showing 156/300 reward rows changed with mean changed-only delta `-0.301`.
- Add retriever-diversity context reward and selector diagnostic reports showing that context-level supervision exposes evidence noise but remains too harsh as a default selector target on the 10-query split.
- Add the retriever-diversity next-run decision report: choose the 50-query, two-budget A1-medium MiMo V2.5 run with `MAX_COMPLETION_TOKENS=2048`, select 100 targeted DeepSeek audit rows from the 300-row subset, and explicitly defer local Qwen profiling from this branch.
- Complete the 100-row targeted DeepSeek v4 Flash context audit for the retriever-diverse subset, with 100 valid labels, 0 invalid JSON/errors, 80/83 MiMo-vs-DeepSeek sufficiency agreement, 76 consensus-insufficient rows, and 3 high-disagreement rows.
- Complete the A1-medium retriever-diverse standard MiMo V2.5 generation gate: 50 SciFact queries, 3 retrievers, 5 policy/profile variants, and budgets `1000,4000` produced 1500/1500 non-empty generations with 0 generation errors and 1500 normalized RLAIF action rows; document the generation-only result in `docs/reports/phase1d_retriever_diversity_a1_medium_generation_validation.md`.
- Add A1 postprocess tooling for sharded Kaggle answer labels and context-label budgeting: `scripts/validate_rlaif_answer_labels.py` merges corrupted/overlapping answer-label shards without turning missing labels into zero scores, `scripts/select_stratified_rlaif_actions.py` prepares a balanced 600-row context-label subset, and `rlaif-eval` now reports selected retriever/policy/profile/budget distributions for retriever-allocation diagnostics.
- Complete A1-medium answer-level RLAIF postprocess: merge three Kaggle MiMo V2.5 answer-label shards into 1500/1500 valid labels with 1460 clean usable labels, rebuild answer-only rewards with 1460 scored rows and 17026 preferences, run a six-seed selector sweep, prepare the balanced 600-row A1 context-label subset, and publish `docs/reports/phase1d_retriever_diversity_a1_mimo_v25_eval.md`.
- Update the internship PDF Main Results with A1 answer-label validation, answer quality by retriever, A1 answer-only selector sweep, and prepared 600-row context-label subset tables; at this checkpoint the final retriever-quality claim was deferred until the later A1 stratified context-label pass.
- Complete the A1 stratified 600-row context-label pass on Kaggle with MiMo V2.5: merge four 150-row shards into 600/600 context labels, validate 548 clean usable labels with no missing/unknown/duplicate action ids, run evidence-quality by retriever/policy, build the non-default context-penalty-0.25 reward candidate with 1492 scored rewards and 20934 preferences, rerun six-seed selector diagnostics, and update the A1 report/PDF with the answer-vs-context retriever-signal divergence.
...
- Polish the internship submission narrative with an explicit contributions subsection, related-work positioning table, phase timeline, headline findings table, clearer HotpotQA/multi-judge framing, and more compact result tables.
- Consolidate detailed Markdown experiment reports into the internship PDF: add data-rich Key Findings, phase timeline, full context-label, reward-ablation, pairwise audit, DeepSeek audit, selector, retriever-diversity, and report-source-map sections; the initial consolidation predated the completed A1 MiMo V2.5 answer labels and the stratified 600-row context-label pass.
- Polish the consolidated internship PDF for submission: clarify that the two pairwise MiMo50 non-decision rows are ambiguous/timeouts rather than separately counted API errors, and add explicit selector/retriever-diversity takeaway sentences.
