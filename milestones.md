...
- Download and summarize the clean `hp-mimo-old-rerun-0603` HotpotQA sampled rerun: 800/800 MiMo generations completed, 0 generation errors, and 80 MiMo-backed RAGAS samples are now included in the HotpotQA report.
- Download and summarize `hp-mimo-evidence-1k32k-20260603-1948`: 300/300 MiMo evidence-aware generations completed, 0 generation errors, and 30 RAGAS samples cover 1k, 2k, 4k, 8k, 16k, and 32k budgets.
- Download and summarize `hp-mimo-lowcurve-500-3k-20260604-0010`: 750/750 MiMo low-budget generations completed, 0 generation errors, and 75 RAGAS samples cover legacy plus adaptive-heuristic balanced/aggressive budgets from 500 to 3000 characters.
- Download and summarize `hp-mimo-adaptive-lowcurve-500-3k-20260604-0037` as an adaptive-only resilience shard: 500/500 generations completed, 0 generation errors, and 50 RAGAS samples confirm the adaptive low-budget pattern.
- Split the HotpotQA report into clean MiMo result summary and diagnostic/quota-contaminated Groq runs so the Groq 429 outcome is not read as the benchmark conclusion.
...
- Submit a fixed Groq `qwen/qwen3-32b` HotpotQA retry kernel after the first retry failed before generation due to a missing Kaggle dataset attachment; the fixed kernel attaches the original-output dataset, auto-discovers the input directory, uses conservative one-key Groq pacing, and recomputes MiMo-backed RAGAS with `n=20/action`.
