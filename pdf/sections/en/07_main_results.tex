...
A1 answer-level quality is now measured. & A1 has 1500/1500 valid MiMo V2.5 answer labels, 1460 clean usable labels, 1460 scored rewards, and graph-BM25 has the best mean answer-level reward among the three retrievers. & This is an answer-level signal; final retriever-quality ranking still needs context labels. \\
A1 selector signal is positive but costly. & In the A1 answer-only sweep, \code{linear\_reward\_model} is the strongest non-oracle selector: reward 0.734 $\pm$ 0.032, quality 0.987 $\pm$ 0.018, oracle gap 0.081 $\pm$ 0.032. & The learned selector uses high-cost actions; context labels are still needed to judge evidence quality. \\
\bottomrule
\end{tabular}
\end{center}
...
Retriever-diversity extension & Multi-retriever logged actions & BM25, graph-BM25, and hybrid-RRF retrieval-only and generation matrices. & Retrieval-only 2250-row coverage run; A1-medium 1500-row generation gate with complete answer labels and context labels pending. \\
\bottomrule
\end{longtable}
}
...
\begin{tabular}{l r}
\toprule
A1 context-label subset metric & Value \\
\midrule
Source action rows & 1500 \\
Answer labels available & 1500 \\
Stratification cells & 30 \\
Selected context-label rows & 600 \\
Rows per cell & 20 \\
Underfilled cells & 0 \\
\bottomrule
\end{tabular}
...
\textbf{Takeaway:} the retriever-diversity extension has fixed the logged-action coverage problem and now has answer-level quality labels. Graph-BM25 is the strongest mean answer-level retriever in A1. In the answer-only selector sweep, \code{linear\_reward\_model} is the strongest non-oracle selector by reward and oracle gap, but it tends to select high token/KV-cost actions. The context-supervision gap remains:

\begin{quote}
Retriever diversity coverage and answer-level A1 evaluation are now complete, but final retriever-quality evidence is still in progress. The 600-row stratified A1 context-label subset must be labeled before claiming a stable BM25 vs graph-BM25 vs hybrid-RRF ranking.
\end{quote}
