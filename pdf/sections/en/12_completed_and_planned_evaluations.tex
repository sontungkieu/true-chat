...
Current evidence mostly supports context-budget/action selection. To claim retrieval-context allocation, logged actions must include multiple retrieval strategies. A first retrieval-only SciFact run now covers 45 jobs over 50 queries:
...
The next step is a small MiMo-\code{v2.5} generation subset with explicit \code{retriever}, \code{context\_policy}, \code{adaptive\_profile}, \code{budget}, \code{generator\_model}, and \code{judge\_model} columns. Optional web-search remains a live stress test only and must not be mixed with reproducible BEIR claims.
