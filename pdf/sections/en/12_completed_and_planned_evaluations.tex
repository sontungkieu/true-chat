...
The A1-medium generation and answer-label gate is now complete. It covers 50 SciFact queries, BM25, graph-BM25, hybrid-RRF, five policy/profile variants, budgets \code{1000} and \code{4000}, and standard \code{mimo-v2.5} with \code{MAX\_COMPLETION\_TOKENS=2048}. The run produced 1500/1500 non-empty generated answers, 0 generation errors, 30 query-result files, and 1500 normalized RLAIF action rows. The three Kaggle answer-label shards merged into 1500/1500 valid MiMo V2.5 answer labels, with 1460 clean usable labels, 40 ambiguous labels, 0 invalid JSON lines, no missing action ids, no unknown action ids, and no duplicate conflicts.

The answer-only reward rebuild produced 1500 reward rows, 1460 scored rewards, and 17026 preference pairs. The first answer-level retriever signal favors graph-BM25 over BM25 and hybrid-RRF on mean quality/reward, while hybrid-RRF remains noisier under the current implementation. A 600-row context-label subset has been selected with 20 rows in each retriever-policy-profile-budget cell. This means A1 answer-level supervision is no longer pending, but context-level A1 evidence supervision remains the next blocker before making a final retriever-quality claim.
