...
\begin{itemize}[leftmargin=*]
  ...
  \item Separate validation and answer-quality diagnostics now report generation coverage, missing-answer rate, answer quality, support, unsupported-claim risk, and cost by retriever and context policy.
  \item Full context-level MiMo judging now covers the same 300 action rows: 300 valid JSON context labels, 253 clean usable labels, 134 sufficient contexts, 158 insufficient contexts, mean context quality 0.505, and mean evidence support 0.436.
  \item A non-default context reward candidate changed 156/300 reward rows, mostly downward, and increased preference pairs from 1559 to 2412. This confirms that context labels add evidence-quality supervision, but the candidate remains a calibration target rather than a default selector target.
...
The next-run decision therefore keeps the retriever-diverse branch active but avoids the full 2250-row matrix for now. The recommended follow-up was a 50-query, two-budget run with standard MiMo V2.5 and \code{MAX\_COMPLETION\_TOKENS=2048}, plus a targeted DeepSeek audit over 100 high-impact context rows. Local Qwen profiling is deferred from this branch.
...
The A1-medium generation gate is now complete. It covers 50 SciFact queries, BM25, graph-BM25, hybrid-RRF, five policy/profile variants, budgets \code{1000} and \code{4000}, and standard \code{mimo-v2.5} with \code{MAX\_COMPLETION\_TOKENS=2048}. The run produced 1500/1500 non-empty generated answers, 0 generation errors, 30 query-result files, and 1500 normalized RLAIF action rows. Mean answer length is 338 characters and mean total latency is 6.46 seconds. This result closes the generation-coverage issue from the earlier low-cap subset, but it is not yet an answer-quality or selector-quality result because all 1500 feedback rows still have provenance \code{missing} until answer/context judge labels are run.

The postprocess path for A1 is prepared before the Kaggle answer-label shards return. A dedicated validator merges sharded MiMo answer-label JSONL files, skips corrupted partial lines from interrupted jobs, reports missing/unknown/duplicate action ids, and preserves ambiguous or missing labels instead of converting them to zero. A stratified sampler then selects a 600-row context-label subset balanced by retriever, policy/profile, and budget, with optional priority for ambiguous, low-support, unsupported-risk, high-quality-low-support, and high-cost rows. Selector reports now include selected retriever, context-policy, adaptive-profile, and budget distributions so A1 can test retrieval-context allocation rather than only scalar reward.
