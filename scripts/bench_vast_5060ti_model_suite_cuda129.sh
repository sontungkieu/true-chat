...
for model in "${models[@]}"; do
  ...
  vast_log_step "model ${model_index}/${total_models}: ${model} preset=${preset} cuda=12.9 kv_cache_dtype=${model_kv_cache_dtype:-auto}"
  if ! BENCH_VLLM_KV_CACHE_DTYPE="$model_kv_cache_dtype" scripts/bench_vast_5060ti_cuda129.sh "$model" "$preset" "$@"; then
    failures=$((failures + 1))
