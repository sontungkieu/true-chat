...
run_case() {
  ...
  vast_log_step "sd sweep case ${case_index}: cuda=12.9 label=${label} model=${model} preset=${preset} speculative_config=${speculative_config:-off}"
  if ! BENCH_VLLM_SPECULATIVE_CONFIG="$speculative_config" scripts/bench_vast_5060ti_cuda129.sh "$model" "$preset" "$@"; then
    failures=$((failures + 1))
...
vast_log_step "sd sweep plan: cuda=12.9 model=$model preset=$preset include_baseline=$include_baseline ngram_tokens='$ngram_tokens' max_model_len=$BENCH_MAX_MODEL_LEN max_num_seqs=$BENCH_MAX_NUM_SEQS max_num_batched_tokens=$BENCH_MAX_NUM_BATCHED_TOKENS enforce_eager=$BENCH_ENFORCE_EAGER"
