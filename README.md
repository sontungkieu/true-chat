...
When a dictionary runtime dataset is attached, the uploader defaults `rag-bench serve` to `--bench none --retriever dictionary-graph --model-id rag-dictionary-graph`, so normal chat requests do not start from the SciFact BM25 corpus. Pass `--serve-bench scifact --serve-retriever bm25` only for a deliberate mixed benchmark-plus-dictionary fallback deployment. See [`docs/data_component_layout.md`](docs/data_component_layout.md) and `configs/corpus_profiles/` for the data/component separation convention.
