...
In `Text only` mode, the optional dictionary fallback also uses the same short lookup normalization as dictionary mode before it accepts dictionary evidence, so low-effort queries such as `PB là gì?`, `pblagi`, `PB viết tắt cho gì?`, `pbviettatcuagi`, `CVHL nghĩa là gì?`, `cvhlnghialagi`, `KHCN xuất hiện ở đâu?`, `khcnxuathienodau`, `CTCC xuất hiện ở đâu?`, `ctccxuathienodau`, or `Q S P B` can fall back to the normalized target instead of searching a stale benchmark context when the selected text retriever has no positive evidence. The dictionary retriever also avoids promoting very short headwords as substring matches inside longer compact lookup strings, so a compact acronym query does not get hijacked by unrelated one-syllable entries. In Dev mode, assistant footers include the active backend commit id so deployed answers can be tied to the exact code revision.
