...
Run offline Red/Blue dictionary autoresearch when you want the system to discover new semantic failure cases before changing the retriever. The Red side reads the local dictionary artifact/source and proposes adversarial queries with expected entries; the Blue side runs `dictionary-graph`, checks retrieval truth, and optionally calls a judge model for answer truth. Semi-private PB dictionary data may use MiMo; private source classifications are blocked unless the provider is `local` and the selected model is explicitly trusted:

```bash
uv run --frozen rag-bench autoresearch-dictionary \
  --artifact-dir runs/pb_dictionary_base_supp2021_prod_graph \
  --source-classification semi-private \
  --provider mimo \
  --model mimo-v2.5-pro \
  --rounds 2 \
  --limit 40
...
Use `--dry-run-model` for retrieval-only smoke tests with no provider calls. Each run writes an isolated folder under `runs/dictionary_autoresearch/<timestamp-or-run-name>/`; reusing the same `--run-name` is treated as an operator error so earlier evidence is not overwritten. The folder contains `cases.jsonl`, `rounds.jsonl`, `failures.jsonl`, `coordinator_decisions.jsonl`, `coordinator_decisions.md`, `summary.md`, `codex_session.md`, and `codex_tasks.md`. `rounds.jsonl` is appended after each evaluated case so interrupted runs still keep partial evidence, and CLI progress is printed to stderr unless `--quiet` is set. Resume an interrupted run with the same config plus `--run-name <run> --resume`; resume reuses `cases.jsonl`, skips completed `(round_index, case_id)` pairs from `rounds.jsonl`, appends the missing evaluations, then finalizes the summary and coordinator files. The answer judge retries invalid JSON with `--judge-json-retries` before marking the case ambiguous, and dictionary answer judging uses the same user-facing answer formatter as production so the raw dictionary entry is counted before optional model explanation. Raw-entry-only answers are judged deterministically: the expected entry id/title passes without a judge call, while a formatted answer headed by the wrong entry id fails immediately. Red generation is evidence-gated: abbreviation cases come from graph aliases or abbreviation tokens tied to the headword initials, and definition phrases must be distinctive within the local corpus. It also mines acronym collisions and emits context probes such as `BĐC tháo súng` versus `BĐC 1951`, while bare generated abbreviations with multiple valid entries are marked ambiguous instead of forcing an impossible single truth. Short abbreviation seed, feedback, and context cases require the expected entry at rank #1 by default; pass `--no-strict-acronym-rank` for softer audit runs. Pass previous runs with repeatable `--feedback-run-dir runs/dictionary_autoresearch/<run>` to prioritize confirmed failures, candidate failures, and near-miss ranks before broad corpus exploration; replayed short queries are mutated into natural lookup forms such as `<term> là gì` and `tra <term>` so wrapper normalization is tested. Rejected coordinator decisions are skipped. The Codex files are meant for one coordinator session: Red/Blue agents only produce evidence, while the coordinator updates decisions to `accepted`, `rejected`, `needs_more_evidence`, or `converted_to_test`, then either assigns a bounded worker task or declines the failure.
