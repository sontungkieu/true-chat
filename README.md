...
In `Text only` mode, the optional dictionary fallback also uses the same short lookup normalization as dictionary mode before it accepts dictionary evidence, so queries such as `PB là gì?`, `CVHL nghĩa là gì?`, or `KHCN xuất hiện ở đâu?` can fall back to the normalized target instead of searching a stale benchmark context when the selected text retriever has no positive evidence. In Dev mode, assistant footers include the active backend commit id so deployed answers can be tied to the exact code revision.
