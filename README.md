...
Offline generator/judge evaluation is available with `rag-bench eval-rag`. It runs the normal RAG generator path, computes deterministic heuristic checks, and optionally calls a separate stronger judge only when the eval item `data_tier` and backend trust policy allow it. External judges are disabled by default, private external judging is blocked before prompts are built, and outputs go under ignored `eval_results/`. Private eval text, judge errors, and free-form judge issue strings are redacted by default; `--include-private-eval-text` is only for protected local debugging. See [`docs/rag_generator_judge_eval.md`](docs/rag_generator_judge_eval.md).
